...
import {
  ...
  RevealData,
  ProofInfo,
} from "../tasks/generate-merkle-proof";
...
describe("merkleERC721 testing suite", function () {
  ...
  beforeEach(async () => {
    ...
    [tree, proofData] = constructMerkleTree(
      tokenDataInput,
      chainId,
      merkleERC.address
    );
  ...
  describe("When merkle root not set", () => {
    ...
    it("Stranger cannot set merkle root", async () => {
      await expect(merkleERC.connect(stranger).setMerkleRoot(tree.getHexRoot()))
        .to.be.reverted;
    });
    ...
    it("Owner can set the merkle root", async () => {
      ...
      await expect(tx)
        .to.emit(merkleERC, "MerkleRootUpdated")
